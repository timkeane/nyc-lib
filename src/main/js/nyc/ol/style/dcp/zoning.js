 ...
 * @namespace
 */
nyc.ol.style.dcp = nyc.ol.style.dcp || {};

/** 
 * @public 
 * @namespace
 */
nyc.ol.style.dcp.zoning = {
	/** 
	 * @public 
	 * @namespace
	 */
	district: {
		...
		polygon: function(feature, resolution){
			var cache = nyc.ol.style.dcp.zoning.district.cache.district;
			...
			if (!cache[zoom][district]){
				var style = new ol.style.Style({
					fill: new ol.style.Fill({
						color: nyc.ol.style.dcp.zoning.district.color[feature.get('CATEGORY')]
					})
				});
				if (zoom > 12){
		...
		label: function(feature, resolution){
			var cache = nyc.ol.style.dcp.zoning.district.cache.label;
			var zoom = nyc.ol.TILE_GRID.getZForResolution(resolution);
			var district = feature.get('ZONEDIST');
			cache[zoom] = cache[zoom] || {};
			if (!cache[zoom][district]){
