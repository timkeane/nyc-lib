...
class MapLocator extends NycMapLocator {
  constructor(options) {
    ...
    const feature = new ol.Feature({name: location.name, isFeature: location.isFeature})
		if (geoJson) {
			feature.setGeometry(format.readGeometry(location.geometry))
		} else {
			feature.setGeometry(new ol.geom.Point(location.coordinate))				 
		}
		return feature
	}
	/**
 ...
 * @const {ol.style.Icon}
 */
MapLocator.LOCATION_STYLE =  new OlStyleIcon({
  scale: 48 / 512,
  size: [1024, 1024],
