 ...
 * @namespace
 */
nyc.ol.style.transportation = nyc.ol.style.transportation || {};

/** 
 * @public 
 * @namespace
 */
nyc.ol.style.transportation = {
	/** 
	 * @public 
	 * @namespace
	 */
	subway: {
		...
		line: function(feature, resolution){
			...
			if (route){
				var cache = nyc.ol.style.transportation.subway.cache.line;
				var zoom = nyc.ol.TILE_GRID.getZForResolution(resolution);
				cache[zoom] = cache[zoom] || {};
				if (!cache[zoom][route]){
					var width = [1, 1, 2, 4, 6, 8, 10, 12, 14, 18, 22, 26, 32, 32][zoom - 8];
					cache[zoom][route] = new ol.style.Style({
						stroke: new ol.style.Stroke({
							color: nyc.ol.style.transportation.subway.color[route],
							width: width
						})
					});
				}
				...
				return nyc.ol.style.transportation.subway.transfer(feature, resolution);
			}
		},
		/**
		 * @desc Style function for Zoning District polygons
		...
		transfer: function(feature, resolution){
			var cache = nyc.ol.style.transportation.subway.cache.transfer;
			var zoom = nyc.ol.TILE_GRID.getZForResolution(resolution);
			if (!cache[zoom]){
		...
		station: function(feature, resolution){
			var cache = nyc.ol.style.transportation.subway.cache.station;
			var zoom = nyc.ol.TILE_GRID.getZForResolution(resolution);
