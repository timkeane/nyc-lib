...
beforeEach(() => {
  ...
  options.facilityMarkerColor = undefined
...
describe('constructor', () => {
  ...
  test('constructor all options', () => {
    ...
    options.facilityMarkerUrl = 'icon.png'
...
describe('loadMarkerImage', () => {
  ...
  test('loadMarkerImage', () => {
    ...
    mapMgr.loadMarkerImage(facilityMarkerUrl, 32, mockStyle)
  
    const test = () => {
      return new Promise(resolve => {
        expect(Icon).toHaveBeenCalledTimes(1)
        expect(Icon.mock.calls[0][0].src).toBe(facilityMarkerUrl)
...
describe('createStyle', () => {
  ...
  test('createStyle no facilityStyle, facilityMarkerUrl supplied', () => {
    expect.assertions(7)
    const loadMarkerImage = MapMgr.prototype.loadMarkerImage
    MapMgr.prototype.loadMarkerImage = jest.fn()

    options.facilityStyle = ''
    options.facilityMarkerUrl = 'facilityMarkerUrl'
    const mapMgr = new MapMgr(options)

    expect(Style.mock.calls[0][0]).toBeUndefined()
    expect(mapMgr.loadMarkerImage).toHaveBeenCalledTimes(1)
    expect(mapMgr.loadMarkerImage.mock.calls[0][0]).toBe(options.facilityMarkerUrl)
    expect(mapMgr.loadMarkerImage.mock.calls[0][1]).toBe(32)
  ...
  test('createStyle no facilityStyle or facilityMarkerUrl supplied', () => {
    expect.assertions(9)

    options.facilityStyle = ''
    options.facilityMarkerUrl = ''
    const mapMgr = new MapMgr(options)
  ...
  test('createStyle no facilityStyle or facilityMarkerUrl supplied, facilityMarkerColor supplied', () => {
    expect.assertions(9)

    options.facilityStyle = ''
    options.facilityMarkerUrl = ''
    options.facilityMarkerColor = [255, 255, 255]
    const mapMgr = new MapMgr(options)

    expect(Style.mock.calls[0][0].image instanceof Circle).toBe(true)
    expect(Style.mock.calls[0][0].image.radius_).toBe(6)
    expect(Style.mock.calls[0][0].image.fill_ instanceof Fill).toBe(true)
    expect(Style.mock.calls[0][0].image.fill_.color_).toBe(`rgba(${options.facilityMarkerColor},.5)`)
    expect(Style.mock.calls[0][0].image.stroke_ instanceof Stroke).toBe(true)
    expect(Style.mock.calls[0][0].image.stroke_.color_).toBe(`rgb(${options.facilityMarkerColor})`)
    expect(Style.mock.calls[0][0].image.stroke_.width_).toBe(2)
    expect(mapMgr.createStyle(options) instanceof Style).toBe(true)
    expect(Style).toHaveBeenCalledTimes(3)
