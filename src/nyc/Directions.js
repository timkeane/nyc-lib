...
class Directions extends Contanier {
  ...
  handleResp(response, status) {
    ...
    setTimeout(() => {
      ...
      if (dir.length === 0) {
        dir = '<h2 class="screen-reader-only dir">Directions</h2>'
      }
      target.prepend(dir).children().first().focus()
    }, 200)
  ...
  routeAlt() {
    ...
    if (!hasTransit && $(this.modeBtn).attr('id') === 'transit') {
      $('<div class="no-transit" aria-live="polite">Only walking directions are available.</div>').insertBefore(this.find('div.route'))
    } else {
      this.find('.no-transit').remove()
    }
  }
  /**
   * @private
...
Directions.HTML = `
<div id="directions">
  <button id="back-to-map" class="btn rad-all btn-back-to-finder">Back to finder</button>
  <div id="dir-tabs">
    <div id="route-tab">
      <div class="fld-lbl">From my location:</div>
      <div id="fld-from"><input class="rad-all" data-msg-key="dir_fld_input" data-msg-attr="placeholder" placeholder="Enter an address..."></div>
      <div>
