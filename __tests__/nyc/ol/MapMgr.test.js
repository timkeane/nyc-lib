...
const mapMgr = new MapMgr(options)
  ...
  mapMgr.loadMarkerImage(mapMarkerUrl, mockStyle)

  const test = () => {
    return new Promise(resolve => {
      expect(Icon).toHaveBeenCalledTimes(1)
      expect(Icon.mock.calls[0][0].src).toBe(mapMarkerUrl)
      expect(Icon.mock.calls[0][0].scale).toBe(32/216)
      expect(Icon.mock.calls[0][0].size[0]).toBe(216)
      expect(Icon.mock.calls[0][0].size[1]).toBe(233)
