...
QUnit.test('project', function(assert){
	...
	assert.deepEqual(geocoder.project(coordinates), proj4('EPSG:2263','EPSG:3857', coordinates));
...
QUnit.test('possible', function(assert){
	...
	assert.deepEqual(
		...
		{
			accuracy: 0,
			coordinates:  proj4('EPSG:2263','EPSG:3857', [999682, 226174]),
...
QUnit.test('search (zip code)', function(assert){
	...
	geocoder.one(nyc.Locate.EventType.GEOCODE, function(data){
		assert.deepEqual(
			...
			{
				type: nyc.Locate.ResultType.GEOCODE,
				coordinates:  proj4('EPSG:2263','EPSG:3857', nyc.Geoclient.ZIP_CODE_POINTS['10038']),
