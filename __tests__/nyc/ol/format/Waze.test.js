...
test('Style', () => {
  ...
  features.forEach(f => {
    const style = Waze.Style(f)
    const color = Waze.TypeColors[f.get('type')] || '#000'
    expect(style instanceof Style).toBe(true)
    if (f.getGeometry().getType() === 'Point') {
      expect(style.getImage().getStroke().getColor()).toBe('#000')
      expect(style.getImage().getRadius()).toBe(5)
      expect(style.getImage().getFill().getColor()).toBe(color)
    } else {
      expect(style.getStroke().getColor()).toBe(color)
