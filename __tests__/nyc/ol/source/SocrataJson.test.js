...
describe('urlFunction', () => {
  test('urlFunction min options', () => {
    ...
    const urlFunction = socrataJson.getUrl()
    
    let url = urlFunction([Infinity, Infinity, Infinity, Infinity], 1, olProjGet('EPSG:3857'))
    expect(url).toBe(`${urlString}?&$limit=2000000`)

    url = urlFunction([1, 2, 3, 4], 1, olProjGet('EPSG:3857'))
    expect(url).toBe(`${urlString}?&$limit=2000000&$where=within_box(the_geom,0.000017966305691174966,0.000008983152841195214,0.00003593261138234993,0.000026949458523585642)`)
  })

  test('urlFunction all options', () => {
    expect.assertions(1)
    ...
    const urlFunction = socrataJson.getUrl()
    const url = urlFunction([1, 2, 3, 4], 1, olProjGet('EPSG:3857'))
    expect(url).toBe(`${urlString}?&$$app_token=not-at-work&$limit=10&$where=within_box(geom,0.000017966305691174966,0.000008983152841195214,0.00003593261138234993,0.000026949458523585642)`)
  })
})
